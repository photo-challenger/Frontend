...
import { KeyboardAvoidingView, Platform, TouchableOpacity, Text, Modal, View, Image, ActivityIndicator, Linking } from 'react-native';
...
const LoginScreen = ({ route, navigation }) => {
	...
	const handleLoginFetch = async () => {
		...
		if (response === '존재하지 않는 이메일입니다.' || response === '비밀번호를 다시 확인해 주세요.') {
			showAlert({
				title: "로그인 오류",
				msg: response,
			});
